...
serve(async (req)=>{
  ...
  try {
    ...
    console.log('Fetching all payments for order:', orderId);
    // Determine API URL based on environment
    const apiUrl = environment === 'PRODUCTION' ? `https://api.cashfree.com/pg/orders/${orderId}/payments` : `https://sandbox.cashfree.com/pg/orders/${orderId}/payments`;
    ...
    if (!response.ok) {
      ...
      throw new Error(paymentsData.message || 'Failed to fetch payment details');
    }
