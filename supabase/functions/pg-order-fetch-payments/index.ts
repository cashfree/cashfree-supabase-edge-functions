...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let orderId = pathParts[pathParts.length - 1]; // Get from path like /pg-order-fetch-payments/order_123
      
      // If not found in path, try query parameter
      if (!orderId || orderId === 'pg-order-fetch-payments') {
      ...
      if (!orderId) {
        return new Response(
          JSON.stringify({ 
            success: false,
            error: 'Missing order_id. Provide it as path parameter (/pg-order-fetch-payments/order_123) or query parameter (?order_id=order_123)' 
          }),
          { 
            status: 400,
      ...
      try {
        ...
        response = await (cashfree as unknown as { PGOrderFetchPayments: (orderId: string) => Promise<{ data?: unknown }> }).PGOrderFetchPayments(orderId);
      } catch {
        // Try v4 static method with API version
        response = await (cashfree as unknown as { PGOrderFetchPayments: (version: string, orderId: string) => Promise<{ data?: unknown }> }).PGOrderFetchPayments("2023-08-01", orderId);
      }
